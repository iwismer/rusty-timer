...
async fn test_export_csv_header_and_rows() {
    ...
    let mut fwd = MockWsClient::connect_with_token(&fwd_url, "fwd-csv-token").await.unwrap();
    ...
    // Export CSV
    let csv_resp = reqwest::get(format!("http://{}/api/v1/streams/{}/export.csv", addr, stream_id))
        .await.unwrap();
    assert_eq!(csv_resp.status(), 200);
    let body = csv_resp.text().await.unwrap();

    let lines: Vec<&str> = body.lines().collect();
    assert!(lines.len() >= 3, "CSV should have header + 2 data rows, got {} lines:\n{}", lines.len(), body);
    // Check header
    assert_eq!(lines[0], "stream_epoch,seq,reader_timestamp,raw_read_line,read_type", "unexpected CSV header");
    // Check first data row
    assert!(lines[1].contains("CSV_LINE_1"), "first row should contain CSV_LINE_1");
    // Check that commas in data are properly quoted (RFC 4180)
    assert!(lines[2].contains("\"CSV,WITH,COMMAS\""), "comma-containing data must be quoted, got: {}", lines[2]);
}
...
async fn test_export_csv_not_found() {
    ...
    let resp = reqwest::get(format!("http://{}/api/v1/streams/00000000-0000-0000-0000-000000000000/export.csv", addr))
        .await.unwrap();
