...
#[tokio::test]
async fn test_announcer_sse_emits_announcer_update() {
    let container = Postgres::default().start().await.unwrap();
    let port = container.get_host_port_ipv4(5432).await.unwrap();
    let db_url = format!("postgres://postgres:postgres@127.0.0.1:{}/postgres", port);
    let pool = server::db::create_pool(&db_url).await;
    ...
    let ws_url = format!("ws://{}/ws/v1/forwarders", addr);
    let mut fwd = MockWsClient::connect_with_token(&ws_url, "announcer-sse-token")
        .await
        .unwrap();
    fwd.send_message(&WsMessage::ForwarderHello(ForwarderHello {
        forwarder_id: "fwd-announcer-sse".to_owned(),
        reader_ips: vec!["192.168.200.1:10000".to_owned()],
        display_name: None,
    }))
    .await
    .unwrap();
    ...
    fwd.send_message(&WsMessage::ForwarderEventBatch(ForwarderEventBatch {
        session_id,
        batch_id: "announcer-b1".to_owned(),
        events: vec![ReadEvent {
            forwarder_id: "fwd-announcer-sse".to_owned(),
            reader_ip: "192.168.200.1:10000".to_owned(),
            stream_epoch: 1,
            seq: 1,
    ...
    assert_eq!(json["row"]["chip_id"].as_str(), Some("000000012345"));
}
