...
pub async fn upsert_cursor(pool: &PgPool, receiver_id: &str, stream_id: Uuid, stream_epoch: i64, last_seq: i64) -> Result<(), sqlx::Error> {
    sqlx::query!(
...
pub async fn fetch_cursor(pool: &PgPool, receiver_id: &str, stream_id: Uuid) -> Result<Option<(i64, i64)>, sqlx::Error> {
    let row = sqlx::query!(
...
pub async fn compute_backlog(pool: &PgPool, stream_id: Uuid, active_receiver_ids: &[String]) -> Result<i64, sqlx::Error> {
    if active_receiver_ids.is_empty() { return Ok(0); }
    let total: i64 = sqlx::query_scalar!("SELECT COUNT(*) FROM events WHERE stream_id = $1", stream_id)
