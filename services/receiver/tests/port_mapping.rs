...
fn override_port_used_instead_of_default() {
    let subs = vec![Subscription {
        ...
        reader_ip: "192.168.1.100".to_owned(),
...
fn collision_marks_both_streams_degraded() {
    let subs = vec![
        ...
        Subscription {
            ...
            reader_ip: "10.0.0.100".to_owned(),
            local_port_override: None,
        },
    ];
    let r = resolve_ports(&subs);
    assert!(matches!(
        r[&stream_key("f1", "192.168.1.100")],
        PortAssignment::Collision { wanted: 10100, .. }
    ));
    assert!(matches!(
        r[&stream_key("f2", "10.0.0.100")],
        PortAssignment::Collision { wanted: 10100, .. }
    ));
}
#[test]
...
fn port_override_from_db_subscription() {
    let db = Db::open_in_memory().unwrap();
    db.save_subscription("f", "192.168.1.100", Some(7777))
        .unwrap();
    let subs = db.load_subscriptions().unwrap();
    let r = resolve_ports(&subs);
    assert_eq!(
