...
//! Accepts local consumer connections, replays frames already durable in
//! SQLite, and forwards new frames as they become durable.
...
mod tests {
    ...
    async fn shutdown_closes_listener() {
        ...
        let port = proxy.port;
        proxy.shutdown();
        // The accept loop observes the watch signal asynchronously; poll until
        // the listener is actually dropped instead of sleeping a fixed amount.
