...
pub struct StreamEpochSummary {
    ...
    /// First seq recorded in this epoch. Receivers use it to translate an
    /// epoch selection into a subscribe-time after_seq floor.
    #[prost(uint64, tag = "3")]
    pub start_seq: u64,
    /// Last seq of a closed epoch; absent while the epoch is open.
...
pub struct ReaderControlResponse {
    ...
    /// Updated current epoch name. Present = the epoch's authoritative name;
    /// absent = the epoch has no name (cleared or never set).
...
pub struct ReaderStatus {
    ...
    /// Forwarder-authoritative current epoch name. Present = the epoch's
    /// authoritative name; absent = the current epoch has no name.
    #[prost(string, optional, tag = "8")]
    pub current_epoch_name: ::core::option::Option<::prost::alloc::string::String>,
...
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct DataF2C {
    #[prost(oneof = "data_f2c::Msg", tags = "1, 2, 3, 4")]
