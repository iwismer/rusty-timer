...
describe('api client', () => {
  it('getProfile calls correct URL', async () => {
    ...
    mockFetch.mockResolvedValue(makeResponse(200, { server_url: 'wss://s.com', token: 'tok', log_level: 'info' }));
    const p = await getProfile();
    expect(mockFetch).toHaveBeenCalledWith('http://127.0.0.1:9090/api/v1/profile', expect.any(Object));
  ...
  it('putProfile sends PUT with body', async () => {
    ...
    await putProfile({ server_url: 'wss://s.com', token: 't', log_level: 'info' });
  ...
  it('getStreams returns streams response', async () => {
    ...
    mockFetch.mockResolvedValue(makeResponse(200, { streams: [], degraded: false, upstream_error: null }));
  ...
  it('getStatus returns status', async () => {
    ...
    mockFetch.mockResolvedValue(makeResponse(200, { connection_state: 'disconnected', local_ok: true, streams_count: 0 }));
  ...
  it('connect accepts 200 or 202', async () => {
    ...
    mockFetch.mockResolvedValue({ ok: false, status: 202, json: async () => ({}), text: async () => '' });
    await expect(connect()).resolves.toBeUndefined();
  });

  it('disconnect accepts 200 or 202', async () => {
    const { disconnect } = await import('./api');
    mockFetch.mockResolvedValue({ ok: false, status: 200, json: async () => ({}), text: async () => '' });
  ...
  it('putSubscriptions sends PUT with subscriptions body', async () => {
    ...
    mockFetch.mockResolvedValue(makeResponse(204, null));
    await putSubscriptions([{ forwarder_id: 'f', reader_ip: '192.168.1.100', local_port_override: null }]);
    expect(mockFetch).toHaveBeenCalledWith(
