...
{#if store.showHelpModal}
  ...
  >
    ...
    >
      ...
      >
        <h2 class="text-base font-semibold text-text-primary m-0">Help</h2>
        <button
          onclick={close}
      ...
      <div class="overflow-y-auto px-4 py-3 flex-1">
        {#each cachedSections as { key, section, context } (context + "/" + key)}
          <div id="help-{key}" class="mb-6">
            ...
            <p class="text-sm text-text-muted mb-3">{section.overview}</p>
            ...
            {#each Object.entries(section.fields) as [fieldKey, field] (fieldKey)}
              ...
              >
                ...
                <p class="text-sm text-text-muted m-0 mb-1">{f.summary}</p>
            ...
            {#if section.tips && section.tips.length > 0}
              <div class="mt-2">
                <h4 class="text-sm font-semibold text-text-muted mb-1">Tips</h4>
                <ul class="list-disc pl-4 text-sm text-text-muted space-y-0.5">
        ...
        {#if cachedSections.length === 0}
          <p class="text-base text-text-muted text-center py-8">
