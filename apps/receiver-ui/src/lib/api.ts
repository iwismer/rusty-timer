...
export async function putProfile(profile: Profile): Promise<void> {
  await apiFetch('/api/v1/profile', { method: 'PUT', body: JSON.stringify(profile) });
}
...
export async function putSubscriptions(subscriptions: SubscriptionItem[]): Promise<void> {
  await apiFetch('/api/v1/subscriptions', { method: 'PUT', body: JSON.stringify({ subscriptions }) });
}
...
export async function connect(): Promise<void> {
  ...
  if (resp.status !== 200 && resp.status !== 202) throw new Error(`connect -> ${resp.status}`);
}

export async function disconnect(): Promise<void> {
  const resp = await fetch(`${BASE}/api/v1/disconnect`, { method: 'POST' });
  if (resp.status !== 200 && resp.status !== 202) throw new Error(`disconnect -> ${resp.status}`);
}
