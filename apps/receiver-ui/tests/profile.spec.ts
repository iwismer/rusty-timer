...
test.describe('profile page', () => {
  ...
  test('can fill in and save profile form', async ({ page }) => {
    await page.goto('/');
    await page.locator('[data-testid="server-url-input"]').fill('wss://test.example.com');
    await page.locator('[data-testid="token-input"]').fill('test-token');
    await page.locator('[data-testid="log-level-select"]').selectOption('debug');
  ...
  test('shows connection state from status', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('[data-testid="connection-state"]')).toBeVisible();
  });
});
