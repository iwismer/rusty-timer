    ...
    await page.route('**/api/v1/streams/stream-uuid-1', async (route) => {
      if (route.request().method() === 'PATCH') {
        ...
        await route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ ...MOCK_STREAMS[0], display_alias: postData.display_alias })
        });
      } else {
...
test.describe('per-stream detail page', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/api/v1/streams/stream-uuid-1/metrics', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await page.route('**/api/v1/streams/stream-uuid-1/reset-epoch', async (route) => {
  ...
  test('renders stream detail heading', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="stream-detail-heading"]')).toBeVisible();
  });
  ...
  test('displays raw_count metric', async ({ page }) => {
    await page.goto('/streams/stream-uuid-1');
    await expect(page.locator('[data-testid="metric-raw-count"]')).toBeVisible();
    await expect(page.locator('[data-testid="metric-raw-count"]')).toContainText('500');
  });

  test('displays dedup_count metric', async ({ page }) => {
    await page.goto('/streams/stream-uuid-1');
    await expect(page.locator('[data-testid="metric-dedup-count"]')).toBeVisible();
    await expect(page.locator('[data-testid="metric-dedup-count"]')).toContainText('480');
  });

  test('displays retransmit_count metric', async ({ page }) => {
    await page.goto('/streams/stream-uuid-1');
    await expect(page.locator('[data-testid="metric-retransmit-count"]')).toBeVisible();
    await expect(page.locator('[data-testid="metric-retransmit-count"]')).toContainText('20');
  });
  ...
  test('displays backlog metric', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="metric-backlog"]')).toContainText('3');
  });
  ...
  test('reset-epoch button calls API and shows confirmation', async ({ page }) => {
    await page.goto('/streams/stream-uuid-1');
    const btn = page.locator('[data-testid="reset-epoch-btn"]');
    await btn.click();
    // After reset, confirmation message appears
    await expect(page.locator('[data-testid="reset-epoch-result"]')).toBeVisible();
  });
});
