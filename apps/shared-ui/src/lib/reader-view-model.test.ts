...
describe('formatEpochCreatedAt', () => {
  ...
  it('formats an epoch creation timestamp as a readable date and time', () => {
    expect(formatEpochCreatedAt(Date.UTC(2026, 6, 5, 7, 57), 'en-US', 'UTC')).toBe(
      'Jul 5, 2026, 7:57 AM'
    );
  });
});
...
describe('advanceEpochWithOptionalName', () => {
  ...
  it('advances and names the new epoch in a single call', async () => {
    ...
    const result = await advanceEpochWithOptionalName(' Race 2 ', async (name) => {
      calls.push(name);
    });
...
describe('computeDownloadPercent', () => {
  ...
  it('returns 100 for complete state', () => {
    expect(
      computeDownloadPercent(
        { state: 'complete', reads_received: 50, progress: 50, total: 50 },
        null
      )
    ).toBe(100);
  });
  ...
  it('uses estimatedReads when available', () => {
    expect(computeDownloadPercent({ state: 'downloading', reads_received: 50 }, 100)).toBe(50);
  });
  it('falls back to progress/total when no estimatedReads', () => {
    expect(computeDownloadPercent({ state: 'downloading', progress: 30, total: 60 }, null)).toBe(
      50
    );
  });
  it('clamps to 100', () => {
    expect(computeDownloadPercent({ state: 'downloading', reads_received: 200 }, 100)).toBe(100);
  });
  it('clamps to 0 for negative-ish edge cases', () => {
    expect(computeDownloadPercent({ state: 'downloading', reads_received: 0 }, 100)).toBe(0);
  });
  it('returns 0 when estimatedReads is 0 and no progress/total', () => {
    expect(computeDownloadPercent({ state: 'downloading', reads_received: 5 }, 0)).toBe(0);
  });
  it('returns 0 when total is 0', () => {
    expect(computeDownloadPercent({ state: 'downloading', progress: 0, total: 0 }, null)).toBe(0);
  });
});
...
describe('parseWallClock', () => {
  it('parses a space-separated wall clock as UTC', () => {
    expect(parseWallClock('2026-07-01 20:31:13')).toBe(Date.UTC(2026, 6, 1, 20, 31, 13));
  });
  it('parses an ISO wall clock with milliseconds as UTC', () => {
    expect(parseWallClock('2026-07-01T20:31:13.500')).toBe(Date.UTC(2026, 6, 1, 20, 31, 13, 500));
  });
  it('round-trips through formatWallClock', () => {
    expect(formatWallClock(parseWallClock('2026-07-01 20:31:13'))).toBe('2026-07-01 20:31:13');
  });
...
describe('computeTickingClock', () => {
  ...
  it('advances the clock by real time elapsed since capture', () => {
    ...
    expect(computeTickingClock(readerBase, 1000, 6000)).toBe('2026-07-01 20:31:18');
  });
  ...
  it('reflects a real drift offset in the derived forwarder clock', () => {
    ...
    expect(computeTickingClock(readerBase, 1000, 1000, 120000)).toBe('2026-07-01 20:33:13');
  });
});
