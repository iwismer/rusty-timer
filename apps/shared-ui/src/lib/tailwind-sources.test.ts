...
describe('shared Tailwind source configuration', () => {
  it('includes shared UI component and library files as scan sources', () => {
    const tokensCssPath = resolve(import.meta.dirname, '../styles/tokens.css');
    const tokensCss = readFileSync(tokensCssPath, 'utf8');

    expect(tokensCss).toContain('@source "../components/**/*.svelte";');
    expect(tokensCss).toContain('@source "../lib/**/*.ts";');
  });
