...
export function computeDownloadPercent(
  ...
  if (download.state !== 'downloading') return download.state === 'complete' ? 100 : 0;

  if (estimatedReads != null && estimatedReads > 0 && download.reads_received != null) {
    return Math.min(100, Math.max(0, Math.round((download.reads_received / estimatedReads) * 100)));
  }

  if (download.total != null && download.total > 0 && download.progress != null) {
    return Math.min(100, Math.max(0, Math.round((download.progress / download.total) * 100)));
  }
...
export function computeElapsedSecondsSince(receivedAt: number, now: number): number {
 ...
 */
export function formatHardwareCode(code: string | number | null | undefined): string {
  if (code == null) return '\u2014';
  let value: number;
