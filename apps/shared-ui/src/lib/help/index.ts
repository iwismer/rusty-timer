...
export function searchHelp(query: string): HelpSearchResult[] {
  ...
  for (const [contextName, context] of Object.entries(CONTEXTS) as [
    HelpContextName,
    HelpContext,
  ][]) {
    for (const [sectionKey, section] of Object.entries(context)) {
      ...
      const sectionMatches =
        section.title.toLowerCase().includes(q) || section.overview.toLowerCase().includes(q);
        ...
        results.push({
          ...
          matchedFields:
            matchedFields.length > 0
              ? matchedFields
              : Object.entries(section.fields).map(([k, f]) => ({ fieldKey: k, field: f })),
          matchedTips,
        });
      }
    }
