...
export const FORWARDER_HELP = {
  general: {
    ...
    fields: {
      display_name: {
        ...
        detailHtml:
          'An optional human-readable name for this forwarder. When set, it appears instead of the forwarder ID in receiver stream lists and server status views. Useful when managing multiple forwarders at a race venue.',
  ...
  p2p: {
    ...
    fields: {
      enabled: {
        ...
        detailHtml:
          'When enabled, receivers connect directly to this forwarder over iroh. Server HTTP is used only for registration, allow-list distribution, and status coordination.',
      ...
      server_url: {
        ...
        detailHtml:
          'The forwarder registers its endpoint and fetches receiver allow-lists from this server URL. Timing reads do not flow through this service; they move directly over iroh between forwarder and receiver.',
        default: 'None',
      ...
      server_token_file: {
        ...
        detailHtml:
          'The forwarder reads this file on startup and uses it to authenticate registration and allow-list requests to the server.',
  ...
  readers: {
    ...
    overview:
      'IPICO reader devices this forwarder connects to. Each entry represents a physical IPICO reader.',
    fields: {
      reader_ip: {
        label: 'IP Address',
        summary: 'Network address of the IPICO reader device.',
        detailHtml:
          'The IP address of the IPICO reader. For a range of readers, the start IP and end octet define multiple readers on the same subnet.',
      ...
      reader_port: {
        ...
        detailHtml:
          'The port used to connect to the IPICO reader. Most IPICO Lite readers use port 10000 by default, while Elite readers use port 10100.',
        default: '10000',
        range: '1-65535',
      },
      enabled: {
        ...
        detailHtml:
          'Toggle reader on or off. Disabled readers are not connected. Use this to temporarily deactivate a reader without removing it from the configuration.',
        default: 'Enabled',
      },
      default_local_port: {
        ...
        detailHtml:
          "The default local forwarding port, calculated as <strong>10000 + the last octet of the reader's IP address</strong>. For example, a reader at 192.168.0.50 gets local port 10050.<br><br>Timing software on the local network can connect to this port to receive reads directly.",
      },
      local_port_override: {
        label: 'Local Port Override',
        summary: 'Optional custom local port, overriding the auto-calculated default.',
        detailHtml:
          'Set a custom local forwarding port instead of the auto-calculated default. Use this when your timing software expects data on a specific port, or when multiple readers would otherwise share the same default port.',
        range: '1-65535',
      },
    },
    tips: [
  ...
  read_mode: {
    ...
    fields: {
      read_mode: {
        ...
        detailHtml:
          'The read mode controls how the IPICO reader processes chip reads before sending them to the forwarder.' +
          '<ul>' +
      ...
      timeout: {
        label: 'Timeout',
        summary:
          'Seconds the reader waits after last detection before finalizing a read (FS/LS mode only).',
        detailHtml:
          'The deduplication timeout in seconds, used only in First/Last Seen mode. After detecting a chip, the reader waits this many seconds for additional detections. If no new detections arrive within the timeout, the read is finalized and sent.<br><br>A shorter timeout means faster reporting but risks splitting a single pass into multiple reads. A longer timeout ensures complete pass detection but adds latency.',
        default: '5',
        range: '1-255 seconds',
        recommended:
          '5 seconds for standard race timing. Use shorter (2-3s) for fast-paced events like cycling sprints. Use longer (8-10s) for crowded starts.',
      },
    },
    tips: [
  ...
  controls: {
    ...
    fields: {
      allow_power_actions: {
        ...
        detailHtml:
          "When enabled, the 'Restart Forwarder Device' and 'Shutdown Forwarder Device' buttons become available. This is a safety guard to prevent accidental power actions on the physical hardware.<br><br>'Restart Forwarder Service' is always available regardless of this setting.",
        default: 'Disabled',
  ...
  dangerous_actions: {
    ...
    fields: {
      restart_service: {
        ...
        detailHtml:
          'Stops and restarts the forwarder service. The forwarder will disconnect from all readers and receivers, then reconnect. No data is lost — any unacknowledged reads are preserved and replayed after reconnection.',
      },
      restart_device: {
        label: 'Restart Forwarder Device',
        summary: 'Reboots the physical forwarder hardware. Requires power actions enabled.',
        detailHtml:
          "Initiates a full reboot of the forwarder device. The device will be offline for 30-60 seconds during restart. Requires 'Allow Restart/Shutdown' to be enabled in Forwarder Controls. All reads are preserved across reboots.",
      },
      shutdown_device: {
        label: 'Shutdown Forwarder Device',
        summary: 'Powers off the physical forwarder hardware. Requires power actions enabled.',
        detailHtml:
          "Initiates a clean shutdown of the forwarder device. The device will need to be physically powered back on. Only use this at the end of a race day or if you need to relocate the hardware. Requires 'Allow Restart/Shutdown' to be enabled in Forwarder Controls.",
      },
    },
    tips: [
  ...
  auth: {
    ...
    fields: {
      token_file: {
        ...
        detailHtml:
          'The file path to the authentication token. The forwarder reads this file on startup and uses it for local control authorization and, when reused by <code>p2p.server_token_file</code>, server API authentication.',
        default: 'None (required for authenticated control APIs)',
        recommended:
          'Use a dedicated token file rather than embedding the token in the config file.',
      },
    },
    tips: [
  ...
  journal: {
    title: 'Journal',
    overview:
      'The journal provides durable storage for chip reads, ensuring no data is lost if receiver connections drop.',
    fields: {
      sqlite_path: {
        label: 'Journal File Path',
        summary: 'File path for the journal storage. Leave empty for in-memory.',
        detailHtml:
          "Path to the file used for the forwarder's durable journal.<br><br>An in-memory journal (empty path) is faster but loses data if the forwarder restarts. A file-based journal persists reads across restarts.",
        default: 'In-memory (empty path)',
        recommended:
          'Always use a file path for race day (e.g. /var/lib/rusty-timer/journal.db). In-memory is only acceptable for testing.',
      },
      prune_watermark_pct: {
        label: 'Storage Cleanup Threshold',
        summary: 'Triggers journal cleanup when storage reaches this fullness percentage.',
        detailHtml:
          'The journal cleans up already-sent reads when storage reaches this percentage of its capacity. Lower values free space sooner, higher values allow the journal to grow larger before cleaning up. Unsent reads are always preserved.',
        default: '80%',
        range: '0-100%',
        recommended:
          '80% works well for most scenarios. Lower to 50% if running on storage-constrained devices.',
      },
    },
    tips: [
  ...
  status_http: {
    title: 'Status HTTP',
    overview:
      'The forwarder exposes a local HTTP endpoint for health monitoring and status checks.',
    fields: {
      bind: {
        label: 'Bind Address',
        summary: 'IP:port the status HTTP listener binds to.',
        detailHtml:
          "The network address and port for the forwarder's built-in status endpoint. Use <code>0.0.0.0</code> to allow access from other devices on the network, or <code>127.0.0.1</code> to restrict access to this device only.",
        default: '0.0.0.0:8080',
        range: 'Valid IP:port combination',
        recommended:
          "0.0.0.0:8080 for standard setups. Use 127.0.0.1:8080 if you don't need to check status from other devices.",
      },
    },
    tips: [
  ...
  ups: {
    title: 'UPS (PiSugar)',
    overview:
      'Configure PiSugar UPS monitoring so the forwarder can report battery state and warn when external power is unplugged.',
    fields: {
      enabled: {
        label: 'Enable UPS Monitoring',
        summary: 'Turns on polling of the PiSugar UPS daemon.',
        detailHtml:
          'When enabled, the forwarder polls the PiSugar daemon for battery percentage, charging state, and external power state. The status page shows battery information and warns when the forwarder is running on battery power.<br><br>Disable this if the forwarder does not have a PiSugar UPS attached.',
        default: 'Disabled',
        recommended:
          'Enable on battery-backed Raspberry Pi forwarders. Leave disabled on devices without PiSugar hardware.',
      },
      daemon_addr: {
        label: 'Daemon Address',
        summary: 'Host and port of the PiSugar daemon.',
        detailHtml:
          'Network address used to read PiSugar UPS telemetry. Leave blank to use the standard local PiSugar daemon address. Use a custom value only if the daemon is exposed on a different host or port.',
      ...
      poll_interval_secs: {
        ...
        detailHtml:
          'Polling more often makes the UI update sooner when power or battery state changes. Polling less often reduces background work. The forwarder also sends UPS status upstream on the heartbeat interval.',
        default: '5 seconds',
        range: '1-60 seconds',
        recommended:
          'Use the default 5-second interval unless you need slower polling on a constrained device.',
      },
      upstream_heartbeat_secs: {
        label: 'Heartbeat Interval',
        summary: 'How often UPS status is sent upstream for coordination/status views.',
        detailHtml:
          'Controls the periodic UPS heartbeat sent to upstream status consumers. The local UI can update from each poll, but upstream dashboards use this heartbeat cadence.',
        default: '60 seconds',
  ...
  status_overview: {
    title: 'Status',
    overview:
      'Live identity and health information for this forwarder. These fields are read-only and reflect the current state of the running service.',
    fields: {
      forwarder_id: {
        label: 'Forwarder ID',
        summary: 'Stable identifier for this forwarder, derived from its authentication token.',
        detailHtml:
          "A unique identifier automatically derived from the forwarder's authentication token (e.g. <code>fwd-3a9f1c2b8e4d07f1</code>). The ID is stable across restarts as long as the token doesn't change. Server and receivers use this ID to identify the forwarder in status and stream lists.",
      },
      version: {
        label: 'Version',
        summary: 'Software version currently running on this forwarder.',
        detailHtml:
          'The version of the forwarder service currently running. Use this to confirm an update has been applied after a service restart.',
      },
      readiness: {
        label: 'Readiness',
  ...
  reader_live: {
    ...
    overview:
      'Real-time statistics and controls for an active reader connection. These values update automatically while the page is open.',
    fields: {
      reads_session: {
        label: 'Reads (Session)',
        summary: 'Number of chip reads received from this reader since the forwarder last started.',
        detailHtml:
          'A running count of chip reads received from this reader since the forwarder service was last started. This counter resets to zero on each service restart. Use this to confirm reads are actively flowing from a reader during the current session.',
      },
      reads_epoch: {
        label: 'Reads (Epoch)',
        summary: 'Chip reads from this reader recorded in the current epoch.',
        detailHtml:
          "The number of chip reads recorded under the reader's current epoch. The count starts at zero when the epoch is advanced and persists across service restarts, so it reflects the full epoch even after a mid-race restart. Use this to track reads for the active race or wave segment.",
      },
      reads_total: {
        label: 'Reads (Total)',
        summary: 'Total chip reads from this reader recorded in the journal, across all sessions.',
        detailHtml:
          'The cumulative count of chip reads from this reader across all sessions. This count persists across service restarts when using a file-based journal. If using an in-memory journal, this matches the session count. Use this to gauge total throughput from a reader over the course of an event.',
      },
      local_port: {
        label: 'Local Port',
        summary:
          "Port on this device where the reader's chip reads are available for local timing software.",
        detailHtml:
          "The forwarder makes reads from this reader available on this port, so timing software on the local network can receive them directly — independent of receiver P2P sessions.<br><br>The port is either auto-calculated as <strong>10000 + the last octet of the reader's IP address</strong> (e.g. a reader at 192.168.0.50 uses port 10050), or a custom value if a Local Port Override is configured.",
      },
      last_seen: {
        label: 'Last Seen',
        summary: 'How long ago the most recent chip read was received from this reader.',
        detailHtml:
          'The time elapsed since the forwarder last received a chip read from this reader. Updates automatically while the page is open.<br><br>Shows <strong>never</strong> if no reads have been received in the current session. A rapidly increasing value while the reader is connected may indicate the timing mat is idle or no chips are in range.',
      },
      current_epoch: {
        label: 'Current Epoch',
        summary: "The active epoch number for this reader's stream.",
        detailHtml:
          "Epochs divide a reader's stream into race or wave segments. The current epoch number is attached to new reads from this reader so receivers can distinguish them from earlier segments.<br><br>Advancing the epoch starts a new segment. Existing reads are not deleted and remain associated with their original epoch.",
      },
      ...
      epoch_name: {
        ...
        detailHtml:
          "Assigns a human-readable name to the reader's current epoch and displays it as the active epoch label. Clearing the field and saving removes the name. The name applies to the current epoch only — after advancing to a new epoch, set a new name to identify it.",
      },
      advance_epoch: {
        label: 'Advance Epoch',
        summary:
          'Starts a new epoch for this reader, separating subsequent reads from previous ones.',
        detailHtml:
          "Advances the reader's stream to a new epoch. All reads from this point forward are recorded under the new epoch, allowing receivers to distinguish them from previous reads.<br><br>Reads already captured in earlier epochs are not deleted and will still be delivered if not yet received. Use this at the start of each race or wave to create a clean separation in the read stream. After advancing, set an epoch name to identify the new segment.",
      },
      clock_drift: {
        label: 'Clock Drift',
      ...
      sync_clock: {
        ...
        detailHtml:
          "Sends the current time from the forwarder to the reader using a precision sync procedure. After the sync completes, the residual drift is reported in milliseconds.<br><br>Accurate chip-read timestamps depend on the reader's clock. Sync the clock before each race — especially after the reader has been powered on for the first time, after a long idle period, or if you notice timestamp anomalies. The button is only available while the reader is connected.",
        recommended:
          'Sync the clock at the start of each race day and again before each race if high timestamp accuracy is required.',
      },
      refresh_reader: {
        label: 'Refresh',
        summary: 'Re-polls the reader for its current status, configuration, and clock.',
        detailHtml:
          'Queries the reader and updates all displayed info: status, read mode, TTO setting, and clock. The forwarder polls this automatically on connect, but you can use Refresh at any time — for example, after changing settings directly on the reader, or to confirm a previous command took effect.',
      },
      recording: {
        label: 'Start / Stop Recording',
        summary: 'Toggles whether the reader is recording chip reads to its onboard storage.',
        detailHtml:
          "Controls the reader's onboard recording state." +
          '<ul>' +
        ...
        recommended:
          'Turn recording on before each race as a safety net. Download and clear records after each event.',
      },
      download_reads: {
        label: 'Download Reads',
        summary: "Downloads all chip reads stored in the reader's onboard memory to the forwarder.",
        detailHtml:
          "Transfers all records stored in the reader's onboard memory to the forwarder. A progress bar shows the download status. Downloaded reads are journaled and replayed to receivers just like live reads.<br><br>This is the primary recovery path after a reader or network outage: use Download Reads once connectivity is restored to retrieve any reads that were captured to onboard storage.<br><br>Only one download can run at a time per reader. After a successful download, use <strong>Clear Records</strong> to free the reader's storage for the next race.",
      },
      clear_records: {
        label: 'Clear Records',
        summary: "Erases all stored records from the reader's onboard memory.",
        detailHtml:
          "Permanently erases all chip reads stored in the reader's internal memory. This takes approximately 10 seconds to complete.<br><br><strong>This action is irreversible.</strong> Always use <strong>Download Reads</strong> first if you need to recover the stored data before clearing. Clear records after each event to ensure the reader's storage is ready for the next race.",
        recommended:
          'Always download reads before clearing. Clear records at the end of each race day.',
      },
    },
    tips: [
