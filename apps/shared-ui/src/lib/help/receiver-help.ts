  ...
  config: {
    ...
    fields: {
      receiver_id: {
        ...
        detailHtml: "A unique string that identifies this receiver to the server. The server uses this ID to track which receiver is connected and manage stream subscriptions. Use a descriptive ID like 'finish-line-pc' or 'timing-tent-a' so operators can identify which receiver is which.",
      ...
      server_url: {
        ...
        detailHtml: "The full WebSocket URL of the server to connect to (e.g. wss://server.example.com:8080/ws/receivers). The receiver maintains a persistent WebSocket connection to the server for real-time data streaming. Use wss:// for encrypted connections.",
        default: "None (required)",
  ...
  streams: {
    ...
    fields: {
      stream_identity: {
        ...
        detailHtml: "Each stream represents a unique data feed from a specific reader on a specific forwarder. Streams are identified by the combination of forwarder ID and reader IP. The display alias (if set) shows the forwarder's friendly name instead of the raw ID.",
      },
      subscribed: {
        label: "Subscribed",
        summary: "Whether the receiver is actively receiving data from this stream.",
        detailHtml: "A subscribed stream actively delivers chip reads to the receiver's local port. Unsubscribing a stream stops local delivery but does NOT stop the forwarder from sending data to the server. Data continues to accumulate on the server and can be replayed later.",
      },
      local_port: {
        label: "Local Port",
        summary: "The local TCP port where reads from this stream are forwarded.",
        detailHtml: "The TCP port on the local machine where the receiver forwards chip reads from this stream. Your timing software should be configured to listen on this port. The default port is calculated as 10000 + the last octet of the reader's IP address. Custom ports can be set via the Admin > Port Overrides page.",
      },
      stream_epoch: {
        label: "Epoch",
        summary: "The current epoch (data segment) the stream is reading from.",
        detailHtml: "An epoch represents a segment of timing data on the server, typically corresponding to a race or timing session. Epochs are numbered sequentially. The stream epoch shows which data segment is currently being delivered. Epoch names (if set) provide human-readable labels.",
      },
      earliest_epoch: {
        label: "Earliest Epoch Override",
        summary: "Skip historical epochs and start receiving from a specific epoch.",
        detailHtml: "In Live mode, you can set an earliest-epoch override to skip older data and only receive reads from a specific epoch onward. This is useful when you only care about the current race and don't want to replay historical data. Clear the override to receive all available epochs.",
      },
    },
    tips: [
