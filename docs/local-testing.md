...
> **Note:** When using `uv run scripts/dev.py`, the receiver profile is configured
> automatically with the dev token and server URL. You can skip the profile and
> connect steps below and go straight to subscribing. To re-run the configuration
> manually: `/tmp/rusty-timer-dev/configure-receiver.sh`
